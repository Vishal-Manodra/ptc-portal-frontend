...
export default function Sidebar({ isCollapsed }) {
  ...
  return (
    ...
    >
      ...
      {/* Main navigation */}
      <nav className={`flex-1 overflow-y-auto ${isCollapsed ? "px-1.5" : "px-3"} py-4 scrollbar-thin`}>
        <div className="space-y-0.5">
          <p
            className={`text-xs font-semibold text-gray-400 uppercase tracking-wider transition-all duration-300 truncate ${
              isCollapsed ? "opacity-0 h-0 overflow-hidden pb-0 mb-0" : "px-3 pb-2 mb-1"
            }`}
          >
            Main
          </p>
        ...
        {/* WhatsApp status */}
        <div className={`mt-6 px-1 transition-all duration-300 ${isCollapsed ? "flex justify-center mt-4" : ""}`}>
          <div
            ...
            title={isCollapsed ? "WhatsApp Bot Active: Clients can message documents" : undefined}
          >
            ...
            <p
              className={`text-[10px] text-green-600 mt-1 transition-all duration-300 truncate ${
                isCollapsed ? "opacity-0 max-w-0 max-h-0 overflow-hidden mt-0" : "opacity-100 max-w-[150px]"
              }`}
            >
      ...
      >
        <div
          className={`flex items-center ${
            isCollapsed ? "flex-col gap-3 justify-center py-2" : "gap-3 px-2 py-2"
        ...
        >
          ...
          <div
            className={`flex-1 min-w-0 transition-all duration-300 ${
              isCollapsed ? "opacity-0 max-w-0 max-h-0 overflow-hidden" : "opacity-100 max-w-[120px]"
            }`}
          >
            <p className="text-sm font-semibold text-gray-900 truncate leading-none mb-1">{user?.name}</p>
            <p className="text-[11px] text-gray-400 capitalize leading-none">{user?.role}</p>
          </div>
          {/* Logout button */}
          <button
            onClick={logout}
